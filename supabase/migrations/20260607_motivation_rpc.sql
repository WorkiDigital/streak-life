-- ============================================================
-- Adiciona lembretes de motivacao pessoal na RPC get_pending_schedules
-- Usuarios com horario_motivacao definido recebem 1 lembrete/dia
-- no horario escolhido, sem estar vinculado a um habito especifico
-- ============================================================
...
AS $$
  ...
  -- Lembrete de motivacao pessoal: 1 por dia, no horario escolhido pelo usuario
  motivacao_due AS (
    SELECT
      p.id AS schedule_id,   -- usa o id do profile como schedule_id unico
      NULL::uuid AS snooze_id,
      p.id AS user_id,
      NULL::uuid AS habit_id,
      p.nome AS nome_usuario,
      'Lembrete de motivacao' AS nome_habito,
      'motivacao' AS categoria,
      p.whatsapp,
      p.push_token,
      ARRAY['push']::text[] AS canais,
      NULL::text AS canal,
      p.tom_preferido AS tom,
      p.timezone,
      ((now() AT TIME ZONE p.timezone)::date) AS data,
      p.horario_motivacao AS horario,
      'motivacao'::text AS source
    FROM public.profiles p
    WHERE p.horario_motivacao IS NOT NULL
      AND p.motivacao_pessoal IS NOT NULL
      AND p.push_token IS NOT NULL
      AND COALESCE(p.silent_mode, FALSE) = FALSE
      AND p.horario_motivacao <= ((now() AT TIME ZONE p.timezone)::time)
      AND p.horario_motivacao > (((now() - interval '16 minutes') AT TIME ZONE p.timezone)::time)
      -- Nao reenviar se ja foi enviado hoje
      AND NOT EXISTS (
        SELECT 1 FROM public.reminders_sent rs
        WHERE rs.user_id = p.id
          AND rs.canal = 'push'
          AND rs.data = ((now() AT TIME ZONE p.timezone)::date)
          AND rs.schedule_id = p.id  -- schedule_id = profile id para motivacao
      )
  )
