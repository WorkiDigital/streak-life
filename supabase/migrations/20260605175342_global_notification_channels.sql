-- ============================================================
-- Use profile notification channels globally for reminder delivery
-- ============================================================
...
AS $$
  WITH base AS (
    SELECT
      ...
      ARRAY(
        SELECT c
        FROM unnest(COALESCE(p.canais_preferidos, ARRAY['push']::text[])) AS c
        WHERE c IN ('push', 'whatsapp')
          AND (c <> 'push' OR NULLIF(p.push_token, '') IS NOT NULL)
          AND (c <> 'whatsapp' OR NULLIF(p.whatsapp, '') IS NOT NULL)
      ) AS canais,
      NULL::text AS canal,
